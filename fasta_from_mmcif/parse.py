...
def parse(*,
  ...
  try:
    ...
    structures = {}
    for index in range(len(full_structure)):
         structures[index]=full_structure[index]
...
if __name__ == "__main__":
    if len(sys.argv)!=4:
       cmd = [sys.argv[0],'protein_name','cif_file','outfile']
       sys.exit(' '.join(cmd))
    
    
    protein_name = sys.argv[1]
    cif_file = os.path.join(sys.argv[2],protein_name+'.cif')
    outfile = os.path.join(sys.argv[3])
    
    with open(cif_file, 'r') as cif_file:
        cif_string = cif_file.read()
    # parse cif string
    parsing_result = parse(
            file_id=sys.argv[2], mmcif_string=cif_string)
    if parsing_result.mmcif_object is not None:
        chain_to_seqres = parsing_result.mmcif_object.chain_to_seqres
        for key in chain_to_seqres:
            print(key, chain_to_seqres[key])
            outfile = os.path.join(sys.argv[3],protein_name+'_'+key+'.fasta')
            with open(outfile,'w') as w:
                w.write('>'+protein_name+'_'+key+'\n')
                w.write(chain_to_seqres[key]+'\n')
